...
In this section, we present two examples that illustrate the most common use-cases. The first example shows how to extract the characteristics of physiological activity during a certain time interval (not necessarily tied to a specific and sudden event). The second example represents an event-related paradigm, in which the interest lies in shorter-term physiological changes related to specific events (see **Figure 1** and **Table 1**). The example datasets are available with the package and can be downloaded using the `data()` function.
